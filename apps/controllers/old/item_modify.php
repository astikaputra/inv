...
class Item_modify extends CI_Controller {
	...
	function index()
	{
		if($this->sys_model->security(13) == true)
		{
			...
			$data['page']='menu';
			$data['content_title'] = "Modify Drugs & Medical Supplies";
	...
	function activate_item($item_id=NULL)
	{
		...
		if($this->item_model->set_activate_item($item_id))
		{
		}
	...
	function deactivate_item($item_id=NULL)
	{
		...
		$logs = array(
			...
			$this->sys_model->logging($logs);
		if($this->item_model->set_deactivate_item($item_id))
		{
		}
	...
	function activate_formularium($item_id=NULL)
	{
		...
		$logs = array(
			...
			$this->sys_model->logging($logs);
		if($this->item_model->set_activate_formularium($item_id))
		{
		}
		
		echo "1";
	}
	function deactivate_formularium($item_id=NULL)
	{
		...
		$logs = array(
			...
			$this->sys_model->logging($logs);
		if($this->item_model->set_deactivate_formularium($item_id))
		{
		}
