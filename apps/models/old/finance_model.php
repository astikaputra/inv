...
SELECT
	...
	(ROUND((ROUND(Cost,0) + ROUND(CostPayer,0) - ((ROUND(Cost,0) + ROUND(CostPayer,0)) * a.discount/100)),0)) AS NetRevenue,
	CASE
WHEN a.discount = 100 THEN
	0
ELSE
	(
		QTY * (
...
LEFT JOIN BIlls c ON c.Id = a.billid
WHERE
	a.billdate BETWEEN '".$startdate."' AND '".$enddate."'
AND a.issettle = 1
AND status <> 'Cancel'
--ORDER BY billdate, RegistCode DESC";
	...
	function get_revenue_excel($startdate,$enddate)
	{
	$this->set_db();
	$conn =	$this->load->database($this->set_db(),true);
	$sql = "
...
--ORDER BY billdate, RegistCode DESC";
	...
	function get_revenue_copy($startdate,$enddate)
	{
	$conn =	$this->load->database($this->set_db(),true);
	$sql = "SELECT DISTINCT billdate, Billcreateddate, BillNo, RegistCode, RegistrationTime, patientName, gender, birthdate, age, Address, LOB,  AxdoctorCode, DoctorName, ReferingDoctor, SubSpecialistName, specialistName, GroupSpecialistName, ROOM, MarginClass, Itemcode, ServiceCode, DescName, LOSName, Qty, ItemPrice, BillAmount, PayerAmount, DoctorFee, Discount, PayerName, PayerType, PaymentMethod, Status, IdentityCardTypes, Nationality, IsRetail, Triage, AdmitDate, PaymentStatus FROM dbo.fn_revenue_report('$datestart', '$dateend') ORDER BY billdate DESC";
	$query =  $conn->query($sql);
	return $query->result();
	}
	
    function get_spec_revenue($startdate,$enddate,$lob,$los)
	{
	$conn =	$this->load->database($this->set_db(),true);
	$sql = "
SELECT
...
ORDER BY billdate, RegistCode DESC";
    ...
    function get_spec_lob_revenue($startdate,$enddate,$lob)
	{
	$conn =	$this->load->database($this->set_db(),true);
	$sql = "
SELECT
...
AND c.RecordDate BETWEEN '".$datestart."' AND  '".$dateend."'";
	...
	$conn = $this->load->database($this->set_db(),true);
	$sql = " 
	EXEC sp_getdoctorfee '$datestart','$dateend'";
	$query =  $conn->query($sql);
	return $query->result();	
	}
	...
	a.BillNo,
	a.billdate,
	ROUND(SUM (a.qty *(a.cost + a.costpayer)),0) AS Amount,
	ROUND(b.TotalAmount * b.Discount/100,0) AS Discount,
	ROUND(ROUND(SUM(a.qty * (a.cost + a.costpayer)),0) - (ROUND(b.TotalAmount,0) * ROUND(b.Discount,0)/100) + ROUND(b.RoundAmount,0),0) AS NetRevenue,
	ROUND(b.RoundAmount,0) AS Rounded,
	a.payertype,
	Replace(a.payername, '-', ' ') Payername,
	a.Status BillStatus
FROM
	revenue a
	LEFT JOIN dbo.Bills b ON b.Id = a.billid
WHERE
	a.billdate BETWEEN '".$datestart."' AND  '".$dateend."' AND Status <> 'Cancel' AND a.IsSettle=1 --and payertype='Self Paid'
...
ORDER BY
	...
	function get_glposting($datestart,$dateend)
	{
	$conn = $this->load->database($this->set_db(),true);
	$sql = "EXEC sp_exec_glposting '$datestart','$dateend'";
	$query = $conn->query($sql);
