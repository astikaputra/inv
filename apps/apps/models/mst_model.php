...
//---------------------------------------------------- Start Item Module -------------------------------------------
	...
	$this->set_db();
	$conn =	$this->load->database($this->set_db(),true);
	$sql = "select a.Id,b.Id,100,1,0 from Items a, Stores b WHERE a.Id = $item_id AND CONVERT(varchar,a.Id)+'-'+CONVERT(varchar,b.Id) NOT IN (select CONVERT(varchar,ItemId)+'-'+CONVERT(varchar,StoreId) from ItemStores)";
	$query =  $conn->query($sql);
	return $conn->trans_status();
	}
	...
	$this->set_db();
	$conn =	$this->load->database($this->set_db(),true);
	$sql = "select a.Id,NULL,b.Id from Items a, RefSpecialist b WHERE a.Id = $item_id AND CONVERT(varchar,a.Id)+'-'+CONVERT(varchar,b.Id) NOT IN (select CONVERT(varchar,ItemId)+'-'+CONVERT(varchar,SpecialistId) from SpecialistChargeables WHERE ItemId IS NOT NULL)";
	$query =  $conn->query($sql);
	return $conn->trans_status();
	}
...
//---------------------------------------------------- Start Service Module --------------------------------------
	...
	$sql = "select Items.Id,Items.CodeValue,Items.NameValue,buy.NameValue as 'uom_buy',sell.NameValue as 'uom_sell',
			...
			CASE items.RecordStatus  WHEN 0 Then 'Not Active' WHEN 1 Then 'Active' ELSE 'Not Set' END AS RecordStatus from Items LEFT JOIN RefUomTypes buy ON Items.BuyUomId=buy.Id LEFT JOIN RefUomTypes sell ON Items.SellUomId=sell.Id 
			";
	$query =  $conn->query($sql);
	return $query->result();
	}
