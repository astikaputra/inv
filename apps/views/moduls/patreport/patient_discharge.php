...
<h3>Patient Discharge Report From <?php echo date('d M Y',strtotime($startdate)); ?> Until <?php echo date('d M Y',strtotime($enddate));?> </h3>
<br /><br />
<div class="tabbable">
              ...
              <tbody>
			    ...
			    <?php 
					foreach ($discharge_data->result() as $data_discharge) { 
					 ...
					 echo '<td>'. date('d-m-Y H:i:s',strtotime($data_discharge->DateOfDischarge)).'</td>';
					 if ($data_discharge->PatientClosedTime == NULL) {
					 echo '<td>'."Doesn't Closed Yet".'</td>';
					 }
					 else{
					 echo '<td>'. date('d-m-Y H:i:s',strtotime($data_discharge->PatientClosedTime)).'</td>';
					 }
					 ...
					 echo '<td>'. $data_discharge->PayerName. '</td>';
					 echo '</tr>';                    
				 }; ?>
              ...
              <tbody>
			    ...
			    <?php 
					foreach ($discharge_data_10->result() as $data_discharge_10) { 
					 ...
					 echo '<td>'. date('d-m-Y H:i:s',strtotime($data_discharge_10->DateOfDischarge)).'</td>';
					 if ($data_discharge->PatientClosedTime == NULL) {
					 echo '<td>'."Doesn't Closed Yet".'</td>';
					 }
					 else{
					 echo '<td>'. date('d-m-Y H:i:s',strtotime($data_discharge_10->PatientClosedTime)).'</td>';
					 }
					 ...
					 echo '<td>'. $data_discharge_10->PayerName. '</td>';
					 echo '</tr>';                    
				 }; ?>
