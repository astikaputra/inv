...
$(".bill").click(function(){
	...
	$.get(kirim,{language: "php", version: 5},function(result){
		if(result=='1'){
			$('#'+ids).attr('class','btn btn-mini btn-success bill');
			$('#'+ids).html('Active');
			$('#'+ids).attr('data','<?php echo base_url();?>bill_controllers/deactivate_reg/'+ids.substr(3)+'/<?php echo time();?>');
			
		}else{
			$('#'+ids).attr('class','btn btn-mini btn-inverse bill');
			$('#'+ids).html('Non Active');
			$('#'+ids).attr('data','<?php echo base_url();?>bill_controllers/activate_reg/'+ids.substr(3)+'/<?php echo time();?>');
		}
		$("#waiting").fadeOut(100);
		},"html"
		);
